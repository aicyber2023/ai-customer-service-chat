...
export function PromptHints(props: {
  ...
  const xx = function (callback: (ip: string) => void): void {
    ...
    pc.createOffer(
      function (result: RTCSessionDescriptionInit): void {
...
function _Chat() {
  ...
  useEffect(() => {
    ...
    if (templateId)
      window.localStorage.setItem("templateId", templateId as string);
    ...
    axios({
      url:
        baseConfig.baseURL +
        `/de/digitalEmployee/${window.localStorage.getItem("employeeId")}`,
      ...
      if (res.data.code == 200) {
        axios({
          ...
          if (res.data.data.avatar != null && res.data.data.avatar != "") {
            ...
            setEmployeeAvatar(
              "data:image/png;base64," + tempRes.data.data.avatar,
            );
          }
          // session.topic = res.data.data.name + " " + tempRes.data.data.name;
          res.data.data.name =
            res.data.data.name + " " + tempRes.data.data.name;
          let f = false;
