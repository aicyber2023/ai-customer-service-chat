...
<iframe
  ...
  allow="clipboard-read; clipboard-write">
    <p>Your browser does not support iframes.</p>
        ...
        setShareLink(baseURL.shareURL + `?createBy=${query.get("createBy") || window.localStorage.getItem("createBy")}`)
        //     获取API字符串
        regeneration(false)
    }, []);
    //一键复制
    const muCopy = async (text: string | undefined, msg: string,isApi:boolean=false) => {
        // navigator clipboard 需要https等安全上下文
        if (navigator.clipboard && window.isSecureContext) {
            // navigator clipboard 向剪贴板写文本
            await navigator.clipboard.writeText(typeof text === "string" ? text : '').then(() => {
                setCopySuccess(true);
                if (isApi){
                    console.log("true")
        ...
        <ErrorBoundary>
            <div className="window-header" data-tauri-drag-region>
                <div className="window-header-title">
                    ...
                    {
                        copySuccess && <div className={styles.copySuccess}>
                        ...
                        <div style={{display: "flex"}}>
                            ...
                            <div>
                                <IconButton
                                    text={"复制访客地址"}
                                    onClick={() => {
                                        muCopy(`${baseURL.shareURL}/#/?isVisitor=1&id=${window.localStorage.getItem("employeeId")}&templateId=${window.localStorage.getItem("templateId")}`, "√复制成功 ")
                                    }}
                                />
                            </div>
                        </div>
