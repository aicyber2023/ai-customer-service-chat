...
export function MaskPage() {
  ...
  // 创建数字客服
  const createChatSession = (m: Mask | undefined) => {
    let f = false;
    for (let i = 0; i < chatStore.sessions.length; i++) {
      // @ts-ignore
      if (chatStore.sessions[i].mask.id == m.id) {
    ...
    if (!f) {
      axios({
        ...
        // @ts-ignore
        m.name = m?.name + " " + res.data.data.name;
              ...
              <div className={styles["mask-item"]} key={m.id}>
                <div className={styles["mask-header"]}>
                  <div className={styles["mask-icon"]}>
                    <img src={"data:image/png;base64," + m.avatar} width={25} />
                    {/*<MaskAvatar mask={m}/>*/}
                  </div>
                ...
                <div className={styles["mask-actions"]}>
                  <IconButton
                    ...
                    onClick={() => {
                      localStorage.setItem("templateId", m.templateId);
                      localStorage.setItem("employeeId", m.id);
                      createChatSession(m);
                      navigate(Path.Chat);
                    }}
                  />
