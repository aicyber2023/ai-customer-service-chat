...
function UpdatePassword(props: { onClose?: () => void }) {
    ...
    const [zhinengItem, setZhinengItem] = useState(["通用", "导游", "客服", "行政人员", "法律顾问", "合同审查员",])
    const _headers = {
    ...
    const ConfirmUpdatePassword = () => {
        ...
        axios({
            url: baseConfig.baseURL+`/de/chat/updateUserPwd?oldPassword=${oledPassword}&newPassword=${newPassword1}`,
            headers: _headers,
        }).then((res) => {
            ...
            >
                <div className={styles["user-prompt-modal"]}>
                    <div className={styles["update-modal"]}>
                        <input type={"password"} value={oledPassword} onChange={(e) => {
                            setOledPassword(e.target.value)
                        }} placeholder={"请输入旧密码"}/>
                        <input type={"password"} value={newPassword1} onChange={(e) => {
                            setNewPassword1(e.target.value)
                        }} placeholder={"请输入新密码"}/>
                        <input type={"password"} value={newPassword2} onChange={(e) => {
                            setNewPassword2(e.target.value)
                        }} placeholder={"再次输入新密码"}/>
                        <input type={"button"} value={"确认修改"} onClick={ConfirmUpdatePassword}/>
                    </div>
                </div>
            </Modal>
...
export function UserSettings() {
    ...
    const getUserInfo = () => {
        axios({
            ...
            setUserAvatar("https://img.zcool.cn/community/01dc1b58ae3d6ca801219c77314f09.png@1280w_1l_2o_100sh.png");
            console.log("err-->",err)
        });
    }
            ...
            <div className={styles["settings"]}>
                ...
                <List>
                    ...
                    <ListItem
                        title={"用户名"}
                    >
                        <input type="text" value={userInfo.nickName} onBlur={uploadUserInfo} maxLength={10} onChange={(e) => {
                            updateNickName({e: e})
