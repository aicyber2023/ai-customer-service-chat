...
export function Loading(props: { noLogo?: boolean,id?:string }) {
    return (
        <div className={styles["loading-content"] + " no-dark"}>
            ...
            {!props.noLogo && <img src={logo.src} alt="" width={40} height={40} style={{borderRadius:"20px"}}/>}
            <LoadingIcon/>
        </div>
    );
}
...
const UserSettings = dynamic(async () => (await import("./userSettings")).UserSettings, {
    loading: () => <Loading noLogo/>,
});
        ...
        >
            ...
            {isTest || isAuth || isVisitor ? (
                <>
                    ...
                    {
                        (isAuth&&(!query.get("isTest")&&!query.get("isVisitor"))) && <SideBar className={isHome ? styles["sidebar-show"] : ""}/>
                    }

                    {//(windowWidth < 560 && isCover)
                         false?
                            <div className={styles["window-content"]} id={SlotID.AppBody} style={{width: '100%'}}>
                                <div>
                                    <img src={cover.src} alt="" style={{width: "100vw", height: "100vh"}}/>
                                </div>
